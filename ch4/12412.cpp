...
{
    for (int i = 0; i < k; i++)
        if (!(removed[i] || strcmp(sid[i], sid[k])))
            return 0;
    return 1;
}
...
void add()
{
    while (1)
    {
        ...
        if (valid(n)) score[n++][4] = score[n][0] + score[n][1] + score[n][2] + score[n][3];
...
double get_course_stat(int c, int  s, int *passed, int *failed)
{
    ...
    *passed = *failed = 0;
    ...
    if (*passed + *failed) return (double) tot / (double) (*passed + *failed);
    return -1;
}

void get_stat(int c, int *cnt)
{
