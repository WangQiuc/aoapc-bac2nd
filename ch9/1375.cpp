...
int n, L, ns, car[maxs], cdr[maxs]; // lisp names for "head" and "tail" 8-);
string rule[maxn], sym[maxs], dp[maxs][maxlen];
...
void add_intermediate_symbol(const string &S)
{
    ...
    int h = ID(S.substr(0, 1)), t = ID(S.substr(1, S.length() - 1));
...
int main()
{
    while (cin >> n >> L && n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            int left = ID(rule[i].substr(0, 1)), right = ID(rule[i].substr(2));
